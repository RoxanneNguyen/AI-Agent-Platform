...
const kindCycle = ["strategy", "research", "monitor"];
...
const elements = {
  ...
  languageMenu: getElement("languageMenu"),
  languageSelect: getElement("languageSelect"),
  tabs: getElement("tabs"),
  tabView: getElement("tabView"),
  tabMeta: getElement("tabMeta"),
  assistantScroll: getElement("assistantScroll"),
  assistantMessages: getElement("assistantMessages"),
  assistantInput: getElement("assistantInput"),
  assistantSend: getElement("assistantSend"),
  assistantHints: getElement("assistantHints"),
  assistantCollapse: getElement("assistantCollapse"),
  employeeInfoButton: getElement("employeeInfoButton"),
};
...
function updateAssistantCollapseLabel() {
...
function getAvailableTabTitles() {
...
function getRecentMessages(limit = agnoConfig.maxMessages) {
...
function parseSuggestionDecision(input) {
  if (!input) {
    return null;
  }
  const { normalized, normalizedPlain } = normalizeLanguageInput(input);
...
function detectSuggestion(input) {
  if (!input) {
    return null;
  }
  const { normalized, normalizedPlain } = normalizeLanguageInput(input);
...
function pickAssistantReply(tabTitle) {
  const options = [
...
async function requestAgno(prompt, tab) {
  if (!agnoConfig.enabled) {
    return { reply: null, error: false, actions: [] };
  }
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), agnoConfig.timeoutMs);
  try {
...
function applyLanguageOptions() {
  Array.from(elements.languageSelect.options).forEach((option) => {
    const label = languageLabels[option.value];
    if (label) {
...
function bindEvents() {
  ...
  elements.assistantHints.querySelectorAll(".hint-chip").forEach((button) => {
...
function init() {
  state.tabs = [];
  const initialTabs = tabDefinitions.map((definition) => ({
